...
class ContractviewerApp:
    ...
    def createGuildBtnCallback(self):
        ...
        result = guildCreator.createGuild(
            settlement, self.isHumanBoolVar.get()
        )
        ...
        if display_info['Existe']:
            display_info['Tamanho'] = result['size']['name']
            display_info['Caracteristica'] = result['Characteristic']['name']
            display_info['Reputação Governo'] = result['reputation']['govern'][
                'name'
            ]
            display_info['Reputacao Populacao'] = result['reputation'][
                'population'
            ]['name']
    ...
    def guildSelectCallback(self, option):
        ...
        if aux == 0:
            ...
            self.guildShowerText.insert(
                tk.END,
                'ESSA GUILDA É REPETIDA EXISTE MAIS DE UMA COM O MESMO NOME\n\r',
            )
            self.guildShowerText.insert(
                tk.END, '               !!!!!!!!!!!!!!!!!!!!!!!!!!!\n\r\n\r'
            )
    ...
    def contractsSelectorCallback(self, option):
        ...
        for i in dataObjective:
            if dataObjective[str(i)]['diceRangeMin'] != 20:
                ...
                display_info[
                    'Objetivo ' + str(amountOfObjectives)
                ] = dataObjective[str(i)]['name']
                display_info['Sub-Objetivo ' + str(amountOfObjectives)] = data[
                    'objective'
                ][dataObjective[str(i)]['objectiveKey']][str(i)]['name']
        ...
        if display_info['Contratante'] == 'Governo':
            display_info['Sub-Contratante'] = data['contractors']['subClass'][
                'name'
            ]
        amountOfObjects = 0
        for i in range(1, data['clause']['amount'] + 1):
            if data['clause'][str(i)]['diceRangeMin'] < 21:
                amountOfObjects += 1
                display_info['Clausula ' + str(amountOfObjects)] = data[
                    'clause'
                ][str(i)]['name']
        amountOfObjects = 0
        for i in range(1, data['preRequirement']['amount'] + 1):
            if data['preRequirement'][str(i)]['diceRangeMin'] < 21:
                amountOfObjects += 1
                display_info['PreRequisito ' + str(amountOfObjects)] = data[
                    'preRequirement'
                ][str(i)]['name']
        ...
        if data['fullLocation'][locationKey]['hasDistrict']:
            amountOfDistrict = 0
            for i in range(
                1, data['fullLocation'][locationKey]['district']['amount'] + 1
            ):
                if (
                    data['fullLocation'][locationKey]['district'][str(i)][
                        'diceRangeMin'
                    ]
                    != 20
                ):
                    amountOfDistrict += 1
                    display_info['Distrito ' + str(amountOfDistrict)] = data[
                        'fullLocation'
                    ][locationKey]['district'][str(i)]['name']
        ...
        for i in antagonistJson:
            if (
                str(i) != 'amount'
                and antagonistJson[str(i)]['diceRangeMin'] != 20
            ):
                amountOfAntagonist += 1
                display_info[
                    'Antagonista ' + str(amountOfAntagonist)
                ] = antagonistJson[str(i)]['name']
                antagonistKey = antagonistJson[str(i)]['antagonistKey']
                display_info[
                    'Sub-Antagonista ' + str(amountOfAntagonist)
                ] = data['fullAntagonist'][antagonistKey][str(i)]['name']

        display_info['Complicacao'] = data['complications']['complication'][
            'name'
        ]
        complicationKey = data['complications']['complication'][
            'complicationKey'
        ]
        display_info['Sub-Complicacao'] = data['complications'][
            complicationKey
        ]['name']
        ...
        display_info['Prazo'] = (
            str(data['dueDate']['amount']['value']) + ' Dias'
        )

        for k in display_info:
            self.contractText.insert(
                tk.END, '{} = {}\n'.format(k, display_info[k])
            )
