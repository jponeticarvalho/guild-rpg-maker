...
class GuildMaker:
    ...
    def createGuild(self, settlementSize, isHumanSettlement):
        ...
        try:
            ...
            print(
                '[GuildMaker] ERROR when creating the folder returning without the guild'
            )
            return 'error'
    ...
    def rollDice(self, diceData):
        ...
        diceResult = diceResult + self.dice.roll(
        ...
        for i in data:
            if isHumanSettlement == data[str(i)]['isHuman']:
                if diceResult in range(
                    data[str(i)]['existRangeMin'],
                    data[str(i)]['existRangeMax'] + 1,
                ):
                    self.exist = True
                    if isHumanSettlement:
                        if diceResult in range(
                            data[str(i)]['matrixRangeMin'],
                            data[str(i)]['matrixRangeMax'] + 1,
                        ):
    ...
    def defGuildReputation(self):
        ...
        for i in data:
            ...
            for j in data[i]:
                if diceResult in range(
                    data[i][str(j)]['diceRangeMin'],
                    data[i][str(j)]['diceRangeMax'] + 1,
                ):
                    self.resourceReputationModifier = (
                        self.resourceReputationModifier
                        + data[i][str(j)]['resourceModifier']
                    )
    ...
    def defGuildEmployees(self):
        ...
        for i in data:
            if diceResult in range(
                ...
                self.goersModifier = (
                    self.goersModifier + data[str(i)]['goersModifier']
                )
        ...
        if self.guildJson['employees']['amount']['hasDice']:
            diceResult = (
                self.dice.roll(
                    self.guildJson['employees']['amount']['diceAmount'],
                    self.guildJson['employees']['amount']['diceType'],
                )
                + self.guildJson['employees']['amount']['diceBonus']
            )
