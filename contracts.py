...
class ContractMaker:
    ...
    def creatContract(self, guildJson):
        ...
        contractPath = (
            'generatedGuild/' + self.guildJson['fileName'] + '/contracts/'
        )
        try:
            os.mkdir(contractPath)
        except Exception:
            print(
                '[ContractMaker] ERROR when creating the folder to the contract returning without the guild'
            )
            # return "error"

        contractNumber = len(os.listdir(contractPath))
        self.contratJson['fileName'] = (
            'contract-' + str(contractNumber) + '.json'
        )
        contractPath = (
            contractPath + 'contract-' + str(contractNumber) + '.json'
        )
    ...
    def rollDice(self, diceData):
        diceResult = 0
        print(
            '[ContractRollDice] Rolling '
            + str(diceData['diceAmount'])
            + 'd'
            + str(diceData['diceType'])
            + '+'
            + str(diceData['diceBonus'])
        )
        diceResult += self.dice.roll(
            diceData['diceAmount'], diceData['diceType']
        )
    ...
    def defDueDate(self):
        ...
        for i in data:
            if diceResult in range(
                data[str(i)]['diceRangeMin'], data[str(i)]['diceRangeMax'] + 1
            ):
                self.contratJson['dueDate'] = data[str(i)]
                self.contratJson['dueDate']['rolledDice'] = diceResult

        diceResult = 0
        diceResult = self.rollDice(
            self.contratJson['dueDate']['amount']['dueDateDice']
        )
        diceResult *= int(
            self.contratJson['dueDate']['amount']['diceMultiplier']
        )
    ...
    def defDifficulty(self):
        ...
        diceResult = self.rollDice(
            self.guildJson['size']['contractDistanceDice']
        )
    ...
    def defDistance(self):
        f = open(
            'json4Names/ContractServiceValueReward/contractDifficulty.json'
        )
        data = json.load(f)
    ...
    def defValeuReward(self):
        ...
        diceResultValue += self.guildJson['reputation']['govern'][
            'rewardModifier'
        ]['valeuMod']
        diceResultReward += self.guildJson['reputation']['govern'][
            'rewardModifier'
        ]['rewardMod']

        diceResultValue += self.guildJson['reputation']['population'][
            'rewardModifier'
        ]['valeuMod']
        diceResultReward += self.guildJson['reputation']['population'][
            'rewardModifier'
        ]['rewardMod']

        diceResultValue += self.guildJson['employees']['rewardModifier'][
            'valeuMod'
        ]
        diceResultReward += self.guildJson['employees']['rewardModifier'][
            'rewardMod'
        ]
    ...
    def defPreRequirements(self):
        f = open(
            'json4Names/ContractServiceValueReward/preRequirementsClause.json'
        )
        data = json.load(f)

        for i in data['diceDefinition']:
            if self.contratJson['value']['rolledDice'] in range(
                data['diceDefinition'][str(i)]['diceRangeMin'],
                data['diceDefinition'][str(i)]['diceRangeMax'] + 1,
            ):
                self.contratJson['preRequirementClauseDice'] = data[
                    'diceDefinition'
                ][str(i)]
        ...
        while preRequimentAmount > 0:
            diceResult = self.rollDice(
                self.contratJson['preRequirementClauseDice']['dice']
            )
            if diceResult < 1:
                diceResult = 1
            ...
            for i in data['preRequirements']:
                if diceResult in range(
                    data['preRequirements'][str(i)]['diceRangeMin'],
                    data['preRequirements'][str(i)]['diceRangeMax'] + 1,
                ):
                    self.contratJson['preRequirement'][
                        str(preRequimentNumber)
                    ] = data['preRequirements'][str(i)]
                    self.contratJson['preRequirement'][
                        str(preRequimentNumber)
                    ]['valueUsed'] = self.contratJson['value']['rolledDice']
                    self.contratJson['preRequirement'][
                        str(preRequimentNumber)
            ...
            if diceResult in range(
                data['preRequirements']['21+']['diceRangeMin'],
                data['preRequirements']['21+']['diceRangeMax'] + 1,
            ):
    ...
    def defClause(self):
        ...
        while clauseAmount > 0:
            ...
            for i in data['clause']:
                if diceResult in range(
                    data['clause'][str(i)]['diceRangeMin'],
                    data['clause'][str(i)]['diceRangeMax'] + 1,
                ):
                    self.contratJson['clause'][str(clauseNumber)] = data[
                        'clause'
                    ][str(i)]
                    self.contratJson['clause'][str(clauseNumber)][
                        'valueUsed'
                    ] = self.contratJson['value']['rolledDice']
                    self.contratJson['clause'][str(clauseNumber)][
                        'rolledDice'
                    ] = diceResult
            self.contratJson['clause']['rolledDice'][
                str(clauseNumber)
            ] = diceResult
            clauseAmount -= 1
            clauseNumber += 1
            ...
            # TODO verificar um jeito que nao fique tao hardcoded
            if diceResult in range(
                data['clause']['21+']['diceRangeMin'],
                data['clause']['21+']['diceRangeMax'] + 1,
            ):
                clauseAmount += 2
    ...
    def updateReward(self):
        ...
        for i in range(1, self.contratJson['clause']['amount'] + 1):
            if (self.contratJson['clause'][str(i)]['diceRangeMin'] != 1) and (
                self.contratJson['clause'][str(i)]['diceRangeMin'] != 21
            ):
                points += 1

        for i in range(1, self.contratJson['preRequirement']['amount'] + 1):
            if (
                self.contratJson['preRequirement'][str(i)]['diceRangeMin'] != 1
            ) and (
                self.contratJson['preRequirement'][str(i)]['diceRangeMin']
                != 21
            ):
                points += 1
        ...
        self.contratJson['reward']['totalAmount'] = (
            self.contratJson['reward']['totalAmount']
            * self.contratJson['difficulty']['rewardMultiplier']
        )
        self.contratJson['value']['totalAmount'] = (
            self.contratJson['value']['totalAmount']
            * self.contratJson['difficulty']['valueMultiplier']
        )

        self.contratJson['reward']['totalAmount'] = int(
            self.contratJson['reward']['totalAmount']
        )
        self.contratJson['value']['totalAmount'] = int(
            self.contratJson['value']['totalAmount']
        )
        pass
    ...
    def defContractor(self):
        ...
        # TODO
        diceResult = self.dice.roll(1, 20)
        diceResult += self.guildJson['reputation']['govern'][
            'contractorModifier'
        ]
        diceResult += self.guildJson['reputation']['population'][
            'contractorModifier'
        ]
        if diceResult <= 0:
            diceResult = 1
        ...
        for i in data['who']:
            if diceResult in range(
                data['who'][str(i)]['diceRangeMin'],
                data['who'][str(i)]['diceRangeMax'] + 1,
            ):
                self.contratJson['contractors'] = data['who'][str(i)]
                self.contratJson['contractors']['rolledDice'] = diceResult

        if self.contratJson['contractors']['hasSubClass']:
            subClassData = data[
                str(self.contratJson['contractors']['subClassName'])
            ]
            diceResult = self.dice.roll(1, 20)
            for i in subClassData:
                if diceResult in range(
                    subClassData[str(i)]['diceRangeMin'],
                    subClassData[str(i)]['diceRangeMax'] + 1,
                ):
                    self.contratJson['contractors']['subClass'] = subClassData[
                        str(i)
                    ]
                    self.contratJson['contractors']['subClass'][
                        'rolledDice'
                    ] = diceResult
    ...
    def defContractType(self):
        ...
        for i in data['contractType']:
            if diceResult in range(
                data['contractType'][str(i)]['diceRangeMin'],
                data['contractType'][str(i)]['diceRangeMax'] + 1,
            ):
    ...
    def defObjective(self):
        ...
        while objectiveAmount > 0:
            ...
            for i in data:
                if diceResult in range(
                    data[str(i)]['diceRangeMin'],
                    data[str(i)]['diceRangeMax'] + 1,
                ):
                    objectiveJson['generalObjectives'][
                        str(objectiveNumber)
                    ] = data[str(i)]
                    objectiveJson['generalObjectives'][str(objectiveNumber)][
                        'rolledDice'
                    ] = diceResult

            # TODO verificar um jeito que nao fique tao hardcoded
            if diceResult in range(
                data['20+']['diceRangeMin'], data['20+']['diceRangeMax'] + 1
            ):
                objectiveAmount += 2
            else:
                if (
                ...
                ][objectiveNumber] = self.rollSubObjective(
                    fullData[
                        objectiveJson['generalObjectives'][
                            str(objectiveNumber)
                        ]['objectiveKey']
    ...
    def rollSubObjective(self, dataJson):
        ...
        for i in dataJson:
            if diceResult in range(
                dataJson[str(i)]['diceRangeMin'],
                dataJson[str(i)]['diceRangeMax'] + 1,
            ):
                subObjectiveJson = dataJson[str(i)]
                subObjectiveJson['rolledDice'] = diceResult

        if subObjectiveJson['needLocation']:
            f = open('json4Names/ContractServiceValueReward/objectives.json')
            fullData = json.load(f)
            ...
            for i in locationJson:
                if diceResult in range(
                    locationJson[str(i)]['diceRangeMin'],
                    locationJson[str(i)]['diceRangeMax'] + 1,
                ):
                    subObjectiveJson['locationOfObjective'] = locationJson[
                        str(i)
                    ]
                    subObjectiveJson['locationOfObjective'][
                        'rolledDice'
                    ] = diceResult
    ...
    def rollSubLocation(self, subLocJson):
        ...
        for i in subLocJson:
            if diceResult in range(
                subLocJson[str(i)]['diceRangeMin'],
                subLocJson[str(i)]['diceRangeMax'] + 1,
            ):
    ...
    def rollDistrict(self):
        ...
        while districtAmount > 0:
            ...
            for i in districtJson:
                if diceResult in range(
                    districtJson[str(i)]['diceRangeMin'],
                    districtJson[str(i)]['diceRangeMax'] + 1,
                ):
            ...
            # TODO verificar um jeito que nao fique tao hardcoded
            if diceResult in range(
                districtJson['20-20']['diceRangeMin'],
                districtJson['20-20']['diceRangeMax'] + 1,
            ):
                districtAmount += 2
    ...
    def defAntagonist(self):
        ...
        while antagonistAmount > 0:
            ...
            for i in antagonistJson:
                if diceResult in range(
                    antagonistJson[str(i)]['diceRangeMin'],
                    antagonistJson[str(i)]['diceRangeMax'] + 1,
                ):
                    resultJson['antagonist'][
                        str(antagonistNumber)
                    ] = antagonistJson[str(i)]
                    resultJson['antagonist'][str(antagonistNumber)][
                        'rolledDice'
                    ] = diceResult

            # TODO verificar um jeito que nao fique tao hardcoded
            if diceResult in range(
                antagonistJson['20-20']['diceRangeMin'],
                antagonistJson['20-20']['diceRangeMax'] + 1,
            ):
                antagonistAmount += 2
            else:
                antagonistKey = resultJson['antagonist'][
                    str(antagonistNumber)
                ]['antagonistKey']

                if antagonistKey not in resultJson:
                    resultJson[antagonistKey] = json.loads('{}')
                resultJson[antagonistKey][
                    antagonistNumber
                ] = self.rollSubAntagonist(fullData[antagonistKey])
    ...
    def rollSubAntagonist(self, subAntagonistJson):
        ...
        for i in subAntagonistJson:
            if diceResult in range(
                subAntagonistJson[str(i)]['diceRangeMin'],
                subAntagonistJson[str(i)]['diceRangeMax'] + 1,
            ):
    ...
    def defComplication(self):
        ...
        for i in complicationJson:
            if diceResult in range(
                complicationJson[str(i)]['diceRangeMin'],
                complicationJson[str(i)]['diceRangeMax'] + 1,
            ):
        ...
        resultJson[complicationKey] = self.rollSubComplication(
            fullData[complicationKey]
        )
    ...
    def rollSubComplication(self, subComplicationJson):
        ...
        for i in subComplicationJson:
            if diceResult in range(
                subComplicationJson[str(i)]['diceRangeMin'],
                subComplicationJson[str(i)]['diceRangeMax'] + 1,
            ):
    ...
    def defVillain(self):
        if not self.contratJson['contractType']['villainNeed']:
            return
